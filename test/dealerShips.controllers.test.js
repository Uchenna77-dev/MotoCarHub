...
describe('DealerShips Controller - GET Endpoints', () => {
  ...
  it('getAllDealerShips should return 200 and list of dealerShips', async () => {
    const dealerShips = [{ name: 'MegaAuto Zone' }];
    collectionStub.find().toArray.resolves(dealerShips);

    await dealerShipsController.getAllDealerShips(req, res);

    expect(res.status.calledOnceWithExactly(200)).to.be.true;
    expect(res.json.calledOnceWithExactly(dealerShips)).to.be.true;
  });
  ...
  it('getDealerShipById should return 404 if not found', async () => {
  ...
  it('getDealerShipById should return 200 and dealerShip', async () => {
    ...
    expect(res.status.calledOnceWithExactly(200)).to.be.true;
    expect(res.json.calledOnceWithExactly(dealerShip)).to.be.true;
  });
});
